...
jest.mock("../../../config/prismaClient", () => ({
  ...
  default: {
    ...
    tripInvitee: { create: jest.fn(), findUnique: jest.fn(), update: jest.fn(), delete: jest.fn() },
...
describe("Create Invite Handler", () => {
    ...
    it("should return 403 if user is not an admin", async () => {
      ...
      expect(jsonMock).toHaveBeenCalledWith({ error: "Only admin can send invites." });
    });
    ...
    it("should return 400 if user is already a member", async () => {
      ...
      (prisma.tripMember.findUnique as jest.Mock).mockResolvedValue({ userId: "2", tripId: 1 });
  
      await createInvite(mockReq as Request, mockRes as Response);
  
      expect(statusMock).toHaveBeenCalledWith(400);
      expect(jsonMock).toHaveBeenCalledWith({ error: "User is already a member of this trip." });
    });
...
describe("Validate Invite Handler", () => {
  ...
  it.each([
    ...
    { overrides: {token: undefined}, expectedStatus: 400, expectedMessage: "Invite not found" },
  ])("should return $expectedStatus when token is $token", async ({ overrides, expectedStatus, expectedMessage }) => {
  ...
  it("should return 403 if user email does not match with invite", async () => {
    ...
    expect(statusMock).toHaveBeenCalledWith(403);
    expect(jsonMock).toHaveBeenCalledWith({ error: `This invite is for ${inviteData.email}. Please log in with that email.` });
  });
...
describe("Accept Invite Handler", () => {
  ...
  it("should return 403 if user email does not match with invite", async () => {
    ...
    expect(statusMock).toHaveBeenCalledWith(403);
    expect(jsonMock).toHaveBeenCalledWith({ error: `This invite is for ${invite.email}. Please log in with that email.` });
  });
  ...
  it("should accept invite successfully", async () => {
    ...
    prisma.$transaction = jest.fn().mockResolvedValue([
        (prisma.tripMember.create as jest.Mock).mockResolvedValue({tripId: 1, userId: "1", role: "member"}),
        (prisma.tripInvitee.update as jest.Mock).mockResolvedValue({...invite, status: 'accepted'}),
    ])
...
describe("Delete Invite Handler", () => {
  ...
  it("should return 403 if user is not a admin", async () => {
    ...
    expect(statusMock).toHaveBeenCalledWith(403);
    expect(jsonMock).toHaveBeenCalledWith({ error: "Only admin can delete invites." });
  });
  ...
  it("should delete invite successfully", async () => {
    ...
    expect(statusMock).toHaveBeenCalledWith(200);
    expect(jsonMock).toHaveBeenCalledWith({ message: "Invite deleted successfully" });
  });
});
