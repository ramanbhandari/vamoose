...
export const updateExpenseHandler = async (req: Request, res: Response) => {
  try {
    ...
    // Validate that the authenticated user is a member of the trip
    const isMember = await getTripMember(tripId, userId);
    if (!isMember) {
      res.status(403).json({ error: 'You are not a member of this trip.' });
      return;
    ...
    // Process paidByEmail: convert it to a user ID if provided.
    let paidByUserId = userId; // default to authenticated user
    if (paidByEmail) {
      const paidByUser = await getUserByEmail(paidByEmail);
      if (!paidByUser) {
    ...
    const updateData = {
      ...
      description,
      paidById: paidByUserId,
      splitAmongUserIds, // This field is optional—if provided, the model will update the shares.
    };

    // Call the model function to update the expense
    const updatedExpense = await updateExpense(tripId, expenseId, updateData);
    ...
    handleControllerError(error, res, 'Error updating expense:');
  }
};
